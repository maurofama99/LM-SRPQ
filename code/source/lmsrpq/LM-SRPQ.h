...
public:
	...
	unsigned int landmarks_count = 0;
	...
	tree_node* add_node(RPQ_tree* tree_pt, unsigned int v, unsigned int state, unsigned int root_ID, tree_node* parent, unsigned int timestamp, unsigned int exp, unsigned int edge_time, bool lm = false) // add a node to a normal tree, bool lm indicating if this node is a landmark.
	{
	...
	tree_node* add_lm_node(RPQ_tree* lm_tree, unsigned int v, unsigned int state, unsigned int root_ID, unsigned int root_state,
		tree_node* parent, unsigned int timestamp, unsigned int exp, unsigned int edge_time, bool lm = false) // add a node to the LM tree .
	{
		...
		// this function expand an LM tree given a new node in it, and record the nodes where the timestamp in the time_info map is updated with updated_nodes,
	{
		...
		while (!q.empty())
		{
			...
			map<int, int> aut_edge = aut->getAllSuccessors(tmp->state); // get the edges acceptable to the src state
			...
			for (auto & suc : sucs)
			{
				...
				unsigned int exp = suc.expiration_time;
		...
		// updated_nodes record the nodes whose timestamp has been updated in the time info map of the landmark, lm_results record the final-state nodes to which the laste path timestamp has been updated for each updated LM tree, it is used in the following normal tree update.
	{
		...
		if (it != v2l_index.end())
		{
			...
			while (cur)
			{
				...
				{
					unsigned int time = min(lm_node->timestamp, iter2->second);
	...
	void insert_edge_lm_tree(unsigned int s, unsigned int d, unsigned int label, unsigned int timestamp, unsigned int exp, unsigned int src_state, unsigned int dst_state, RPQ_tree* lm_tree, unordered_map<unsigned long long, vector<pair<unsigned int, unsigned int> > >& lm_results)
	...
	void get_lm_results(unsigned int lm, unsigned int state, unsigned int lm_time, unordered_map<unsigned int, unsigned int>& updated_results)
	{
		...
		for (int final_state : aut->finalStates) {
		...
		while (!q.empty())
		{
			...
			if (landmarks.find(tmp_info) != landmarks.end()) 
			{
				...
				for (auto & i : vec)
				{
					...
					int dst_state = aut->getNextState(tmp->state, edge_label); // check if we can travel to a dst state
					if (dst_state == -1)
						continue;
					unsigned int time = min(tmp->timestamp, i.timestamp); // compute timestamp of the dst node
					unsigned int exp = i.expiration_time;
	...
	void visit_non_lm_tree(unsigned int s, unsigned int d, unsigned int label, unsigned int timestamp, unsigned int exp, unsigned int src_state, unsigned int dst_state,
	...
	sg_edge * insert_edge_lmsrpq(unsigned int id, unsigned int s, unsigned int d, int label, unsigned int timestamp, unsigned int exp) //  a new edge is inserted.
	{
		...
		vector<pair<int, int> >vec = aut->getStatePairsWithTransition(label); // find all the state paris that can accept this label
		for (auto & i : vec) {
		...
		// we carry out expand following this LM tree rather than traverse the graph.
	{
		...
		while (!q.empty())
		{
			...
			while (child) // scan the child of the lm tree node
			{
				...
				unsigned int exp = child->edge_expiration;
		...
		for (auto cur : original_vec) // we scan the nodes already in the lm tree one by one, try to expand them to add the necessary nodes.
		{
			...
			while (!q.empty()) {
				...
				for (auto & j : vec)
				{
					...
					int dst_state = aut->getNextState(cur->state, j.label);
					if (dst_state == -1)
						continue;
	...
	RPQ_tree* build_lm_tree(unsigned int v, unsigned int state) // this function build new lm tree for a landmark, we use time info in prune and may miss some nodes, we will add them back with above fulfill_new_lm_tree later .
	{
		...
		while (!q.empty())
		{
			...
			map<int, int> aut_edge = aut->getAllSuccessors(tmp->state);
	...
	void dynamic_lm_select(double candidate_rate, double benefit_threshold) // the function to select landmarks, first parameter is the candidate selection rate, usually 0.2, the second is the benefit threshold, usually 1.5 
	{
		...
		for (const auto&[fst, snd]: g->adjacency_list) // fst = vertex, snd = successors (vector  of <vertex, edge>)
		{
			...
			for (auto &[fst, snd] : aut->transitions) // for each state in the automaton
			{
				...
				if (count_presence(id, state, 2)) {  // filter out product graph nodes which appear in less than 2 trees.
					map<unsigned int, unsigned int> degree_map = g->get_src_degree(id);
					degree_sum = 0;
		...
		auto num = static_cast<unsigned int>(scores.size() * candidate_rate);
		if (num >= scores.size()) {
		    num = scores.size() - 1;
		}
 						...
 						for (unsigned long long dst_info : erased)
						{
								if (landmarks.find(dst_info) != landmarks.end()) // if a landmark is deleted, we need to check if it will influence the time info map
							{
								if (forests.find(dst_info) != forests.end())
								{
									...
									{
										unsigned int state2 = dst_iter.first;
	...
	void expire(int current_time, const vector<edge_info>& deleted_edges) //given current time, carry out an expiration in the forest.
	{
